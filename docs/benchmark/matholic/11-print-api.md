...
| 단당 고정 슬롯(스마트정렬, countPerColumn) | **있음** (09-21, 「칸 고정」 기본 켜짐) — 프리셋에서 같은 줄(좌 1·우 3, 좌 2·우 4)이 **같은 높이에서 시작**. 줄 높이 = 그 줄 최대 본문 + 남는 공간 균등. 본문 합이 넘치면 종전 비례 분배 | 대표 09-21 캡처 대조 「차이가 분명히 보인다」 → 종전(비례 분배)은 2번·4번 시작선이 어긋났다. 이제 같다 |
| 번호 `01` 큰 연회색 + 배지 | **있음** (09-21, 「번호 01」 기본 켜짐) — 두 자리, 25.5px, #b4bcc8, 점 없음 | 같음 |
| 시험지 헤더(학원명 가운데 · 「수학영역」 알약 · 제목+N문항 배지 · 큰 세리프 페이지 번호 · 이름 줄 · QR) | **있음** (09-21) — 디자인 갤러리 「시험지형」. QR 만 없음(학생별 세션이 이 인쇄엔 없음) | 같음 |
| 그림 로드 후 재측정 | **있음** (09-21) — 미완료 `<img>` 가 로드되면 한 번 더 잼 (실측: 5쪽 좌단 1,474px > 971px 넘침이 사라짐, 온천중 22문항 7→10쪽) | 매쓰홀릭은 이미지 크기를 서버가 알고 있어 이 문제가 없다 |
...
| 표지 | **있음** (09-19, PR #572) — 글자 표지 4종(미니멀·클래식·밴드·격자) + **이미지 슬롯 2종**(위 60% 이미지 / 전면 이미지+제목 띠). 이미지는 센터별 라이브러리(Storage `uploads/covers/{institute}`)에 올려 고른다 | 대표 09-19: *"표지는 구글에서 제미나이로 얼마든지 감각있는 거 만들어 넣을 수 있으니"* → 디자인을 코드로 수십 종 만들지 않고 **밖에서 만든 A4 이미지를 넣는 슬롯**으로. 항목은 매쓰플랫 표지 데이터(title·subtitle·academyName·studentName, chunk_MakeCustomWorkbook 캡처)와 같게 + 문항수·출제·시험일·점수칸 |
...
**가져올 것**: ①~③ 은 PR #571, ④ 표지는 PR #572 로 끝냈다(전부 기본 꺼짐 — 종전 출력 불변). 남은 것 = 답안지 조판(미확인). 스마트정렬·여백은 이미 있으므로 **안 만든다**.
표지 미포함 범위(의도): 한글(.hwpx) 내보내기 · `/api/exams/{id}/print` 서버 인쇄 경로. 브라우저 인쇄(ExamPaperView)만.

## 8. 답안지(풀이) 조판 실측 (2026-09-20, 해여중 내신대비 25문항, DOM 측정)

미리보기엔 **답안지가 지면(`.paper`)으로 안 뜬다.** 화면 밖(`top:-100000`) 측정 블록에서 재고, 인쇄 때 `usePrintArea` 가 인쇄 루트(`data-print-root`)를 찍어 나머지를 숨기는 구조다. 아래는 그 측정 블록에서 잰 값.

| | 실측 |
|---|---|
| 지면 | 문제지와 같은 종이 · **2단, 단 폭 335px(88.7mm)** · 본문 폭 732px |
| 답안지 헤더 | 「이름 : ㅤㅤ」 14px/500 · `N회차 / 문제풀기 (문제 풀이 및 정답)` 14px/500 #666 · 학습지명 18px/700 · 헤더 블록 136px |
| 푸터 | `1 / 2` 10px — 25문항 풀이가 **2장** |
| 문항 | `ARTICLE > SECTION > (HEADER + MAIN)` — HEADER: 번호(SUIT 24px #666) + **`정답: X` 배지**(11px, bg rgba(0,0,0,.06), radius 3~5px, padding 3px 6px, `::before "정답: "` #666) · MAIN: **풀이 이미지**(`/answers/{md5}.webp`, 원본 1280px → 단 폭 335px, 높이 30~407px) |
| 흐름 | 문제지와 같은 슬롯/2단 배열(이미지라 글자 개념 없음) · `break-inside: auto` |
| 「제출: 」 | 학생별 출력 시 `.user-answer` 배지(테두리 1px #0002) 가 정답 옆에 붙음 |

**우리와 대조**: 우리 해설지(`SolutionView`)는 이미 **번호 + 정답 배지 + 난이도** 헤더 → 성격 같음. 차이는 매쓰홀릭이 이미지 2단 슬롯, 우리는 KaTeX 텍스트 자연 흐름. 가져올 것 없음.

**미확인(남음)**: 「빠른정답」 표의 조판 — 그 컴포넌트는 지면이 그려질 때 지연 로드되는데, 자동화 탭이 뒤에 숨어 있으면 매쓰홀릭이 지면을 안 그린다(`requestAnimationFrame` 정지 → `출력 지면을 준비하고 있어요` 에서 멈춤, 실측). **크롬에서 매쓰홀릭 출력 탭을 앞으로 가져온 상태**에서만 잴 수 있다. 우리 빠른정답은 번호/정답 2열 표(`QuickAnswerView`).
