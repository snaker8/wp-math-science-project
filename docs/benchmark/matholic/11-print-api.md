...
| 하단 영역: 출제단원 · 페이지 번호 · 출제자 · 학습번호 | **페이지 번호만** (`페이지 N`, 8mm 하단, 10px #aaa) | 출제단원·출제자·학습번호 없음 |
| 표지 | 없음 | 대표 "급하지 않다" |
| 테마 패턴(pattern1 겹곡선…) + 색 | 헤더 5종 + 강조색 | 성격 같음 |
| 난이도 텍스트 배지(개념~고난도, 10px #666) | 배점 배지만 | 난이도 배지 없음 — 우리 분류 밴드로 바로 가능 |
...
| 양면(빈 페이지 자동 삽입) · 제본여백 | 없음 | 양면은 짝수 맞춤용 빈 페이지 한 장 — 싸다 |

**가져올 것(우선순위, 실측 후 재정렬)**: ① 난이도 텍스트 배지(분류 밴드 그대로, 비용 0) ② 하단 영역 3종(출제단원·출제자·학습번호) ③ 양면 빈 페이지 ④ 표지(보류). 스마트정렬·여백은 이미 있으므로 **안 만든다**.
