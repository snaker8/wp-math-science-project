...
const SectionHeader: React.FC<{
  ...
  <div className="flex items-center justify-between border-b border-warm-border-soft px-5 py-4">
    <div className="flex items-center gap-3">
      <span className="inline-flex h-7 w-7 items-center justify-center rounded-full text-xs font-semibold bg-[#DDEAFD] text-[#2B6CB0]">
        {number}
      </span>
      <div className="flex flex-col">
        <span className="text-sm font-semibold text-warm-text-primary">{title}</span>
        {subtitle && <span className="text-xs text-warm-text-muted">{subtitle}</span>}
      </div>
    </div>
      ...
      <div className="flex h-full w-full flex-col px-4">
        ...
        <div className="mb-4 flex flex-wrap items-center justify-between gap-4">
          ...
          <div className="flex flex-1 items-center justify-between gap-2">
            ...
            <button
              ...
              className={`
                ...
                ${hasSelectedProblems
                  ? 'bg-gradient-to-r from-blue-300 via-blue-400 to-blue-300 text-blue-900 hover:-translate-y-[1px] hover:from-blue-400 hover:via-blue-500 hover:to-blue-400'
                  : 'bg-gradient-to-r from-blue-200 via-blue-300 to-blue-100 text-blue-900 opacity-50 cursor-not-allowed pointer-events-none'
                }
              `}
            >
          ...
          <button
            ...
            className="inline-flex h-10 w-10 items-center justify-center rounded-md border border-gray-300 bg-transparent text-gray-700 hover:bg-gray-50 active:bg-gray-100 transition-all"
          >
        ...
        <div className="flex-1 overflow-hidden pb-2">
          <main className="grid grid-cols-12 gap-4 font-pretendard h-full">
                  ...
                  rightContent={
                    <div className="relative">
                      ...
                      <input
                        ...
                        className="h-8 w-48 rounded-md border border-warm-border-soft bg-transparent pl-9 pr-3 text-sm placeholder:text-warm-text-muted focus:outline-none focus:ring-1 focus:ring-warm-border-accent"
                      />
                    </div>
                  }
                />
                    ...
                    {filteredTextbooks.length > 0 ? (
                      filteredTextbooks.map((textbook) => (
                        <div
                          ...
                          `}
                        >
                          <span className="text-sm text-warm-text-primary">{textbook.name}</span>
                          <span className="text-sm text-warm-text-muted">{textbook.unitCount}</span>
                        </div>
                      ))
                    ) : (
                    ...
                    <div className="divide-y divide-warm-surface-strong">
                      {mockUnits.map((unit) => (
                        <div
                          ...
                          `}
                        >
                          <span className="w-12 text-center text-sm text-warm-text-muted">
                            {unit.order}
                          </span>
                          <span className="flex-1 pl-4 text-sm text-warm-text-primary">
                            {unit.name}
                          </span>
                          <span className="w-16 text-center text-sm text-warm-text-muted">
                            {unit.problemCount}
                          </span>
                        </div>
                      ))}
            ...
            <section className="col-span-12 lg:col-span-6 h-[calc(100vh-220px)] min-h-[560px]">
              <div className="flex h-full w-full flex-col overflow-hidden rounded-2xl border border-warm-border-soft bg-white/95">
                {selectedUnit ? (
                  <>
                    <SectionHeader
                      number={3}
                    ...
                    <div className="flex-1 overflow-y-auto p-5">
                      <div className="space-y-4">
                        {mockProblems.map((problem) => (
                          <div
                            key={problem.id}
                            className="rounded-xl border border-warm-border-soft bg-white p-4 hover:border-warm-border-accent transition-colors"
                          >
                            <div className="flex items-start gap-3">
                              <span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-warm-surface-strong text-sm font-semibold text-warm-text-secondary">
                                {problem.number}
                              </span>
