...
3. ★ 같은 문제 안 여러 그래프는 한 묶음(하나의 bbox)으로 잡을 것.
   예: y=f(x) 와 y=g(x) 가 좌우로 나란히 있으면 둘을 모두 포함하는 큰 bbox 1개 반환.
   각각 분리해서 잡으면 그래프 끝이 잘리는 사고 발생.
4. 한 그래프(또는 그래프 묶음) 안에서 영역을 쪼개지 말 것.
5. 그래프와 표가 따로 있으면 별도 bbox (다른 type).
