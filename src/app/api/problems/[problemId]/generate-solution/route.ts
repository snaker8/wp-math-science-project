...
export async function POST(
  ...
  try {
    ...
    // ★ 해설 분량은 난이도에 붙인다 — 아래 lengthRule 에서 쓴다 (원래 선언은 한참 뒤라 여기서 한 번 더 읽는다)
    const diffLevel = parseInt(String(classification?.difficulty || '3'), 10) || 3;
...
★ 위 그래프/도형 정보를 반드시 참고하여 풀이하세요. 그래프가 없다고 말하지 마세요.`;
    ...
    // ★ 분량은 **문제에 붙인다** (대표 2026-09-15):
    //   "중등 과정에 길어야 할 것은 길어야 하는데, 매쓰플랫·매쓰홀릭 수준의 해설 길이가 합당하다."
    //   앞서 중등 일괄 "3~6단계"로 묶었던 건 틀렸다 — 쉬운 문제도 길고 어려운 문제도 잘린다.
    //   기준을 바꾼다: **시중 교재 해설지(정석·마플)가 아니라 학습지 해설**이다.
    //   교재 해설지는 혼자 공부하는 독자를 상대로 개념부터 쌓지만,
    //   학습지 해설은 수업에서 이미 배운 학생이 **막힌 자리만 확인**하는 물건이다.
    const lengthRule = (() => {
      if (isDescriptive) {
        return `\n\n★ 분량 — 서술형이다. 소문항마다 풀이가 필요하니 **필요한 만큼 쓴다**. 다만 한 단계는 한두 문장이고, 같은 계산을 다시 적지 않는다.`;
      }
      if (diffLevel <= 2) {
        return `\n\n★ 분량 — 개념 확인 수준(난이도 ${diffLevel})이다. **1~2단계**면 끝난다.
- concept 은 빈 문자열("")로 둔다. 한 줄로 끝나는 문제에 개념 설명을 붙이지 않는다.
- tip 도 빈 문자열("")로 둔다.`;
      }
      if (diffLevel <= 5) {
        return `\n\n★ 분량 — 표준 문제(난이도 ${diffLevel})다. **2~4단계**, 한 단계는 한두 문장.
- concept 은 **한 줄**. 쓸 말이 없으면 빈 문자열("").
- 객관식이면 정답 근거를 먼저 밝히고, **오답 보기는 헷갈리는 것만 한 줄씩**. 다섯 개를 전부 검토하지 않는다.
- tip 은 이 유형에서 정말 반복되는 실수가 있을 때만. 없으면 빈 문자열("").`;
      }
      return `\n\n★ 분량 — 어려운 문제(난이도 ${diffLevel})다. **막히는 자리를 충분히** 풀어 쓴다.
- 발상이 필요한 지점(보조선·치환·경우 나누기)은 왜 그렇게 잡는지 한 문장 덧붙인다. 거기가 이 문제의 전부다.
- 그렇다고 계산을 두 번 적지는 않는다.`;
    })();
    const lengthInstruction = lengthRule;
...
★★★ 모범 해설 예시 — **학습지 해설** 수준 ★★★
우리가 만드는 건 시중 교재 해설지(정석·마플)가 아니라 **학습지 해설**이다.
교재 해설지는 혼자 공부하는 독자를 상대로 개념부터 쌓지만, 학습지 해설은 수업에서 배운 학생이
**자기가 막힌 자리만 확인**하는 물건이다. 그래서 분량이 문제 난이도에 비례한다 —
쉬운 문제는 두 줄로 끝나고, 어려운 문제는 발상이 필요한 지점을 충분히 풀어 쓴다.

★ 아래 예시들은 **난이도 5~6짜리** 문제의 분량이다. 난이도 1~2 문제를 이 길이로 쓰면 안 된다.

──── 예시 0: 쉬운 문제 (난이도 2) — 이 정도면 충분하다 ────
문제: 점 $A(-3, 1)$은 제몇 사분면 위의 점인가?

모범 해설:
- concept: ""   ← 쉬운 문제엔 개념 설명을 붙이지 않는다
- step 1: "$x$좌표가 음수, $y$좌표가 양수이므로 제2사분면 위의 점이다."
- finalAnswer: "②"
- tip: ""
★ 이런 문제에 [개념] 문단과 팁을 붙이면 학생이 읽을 것이 늘기만 한다.
...
`;
    ...
    const solutionPrompt = isDescriptive ? `**학습지 해설**을 작성하세요. 수업에서 배운 학생이 자기가 막힌 자리를 확인하는 글입니다.
서술형이라 소문항마다 풀이가 필요하니 필요한 만큼 쓰되, 교재 해설지처럼 개념부터 쌓지는 마세요.
${STRATEGY_GUIDE}
${FEW_SHOT_EXAMPLES}
${curriculumContext}
...
JSON:
{
  ...
  "tip": "이 유형 핵심 포인트 (1줄)"
}` : `**학습지 해설**을 작성하세요. 시중 교재 해설지(정석·마플)가 아닙니다.
수업에서 배운 학생이 **막힌 자리만 확인**하는 글이라, 분량은 문제 난이도에 비례합니다.
${STRATEGY_GUIDE}
${FEW_SHOT_EXAMPLES}
${curriculumContext}
    ...
    if (ANTHROPIC_API_KEY) {
      try {
        ...
        systemPrompt = subject
          ? `당신은 한국 "${subject}" 과목 **학습지 해설**을 쓰는 전문가입니다. 반드시 ${subject} 교육과정 범위 내의 개념만 사용하여 풀이하세요. 상위 과정 개념 사용을 금지합니다. 쉬운 문제는 두 줄로 끝내고 어려운 문제는 막히는 자리를 충분히 풀어 씁니다 — 분량을 일정하게 맞추지 마세요. 반드시 유효한 JSON으로만 응답하세요. LaTeX 수식은 반드시 이중 백슬래시(\\\\)를 사용하세요.`
          : '당신은 한국 수학 **학습지 해설**을 쓰는 전문가입니다. 쉬운 문제는 두 줄로 끝내고 어려운 문제는 막히는 자리를 충분히 풀어 씁니다 — 분량을 일정하게 맞추지 마세요. 반드시 유효한 JSON으로만 응답하세요. LaTeX 수식은 반드시 이중 백슬래시(\\\\)를 사용하세요.';
    ...
    if (!solution && OPENAI_API_KEY) {
      ...
      try {
        ...
        const gptSystemMsg = subject
          ? `당신은 한국 "${subject}" 과목 **학습지 해설**을 쓰는 전문가입니다. 반드시 ${subject} 교육과정 범위 내의 개념만 사용하세요. 분량은 문제 난이도에 비례하게 — 쉬운 문제에 긴 해설을 붙이지 마세요. 반드시 유효한 JSON으로만 응답하세요.`
          : '당신은 한국 수학 **학습지 해설**을 쓰는 전문가입니다. 분량은 문제 난이도에 비례하게 — 쉬운 문제에 긴 해설을 붙이지 마세요. 반드시 유효한 JSON으로만 응답하세요.';
...
JSON: { "finalAnswer": "최종 정답", "reasoning": "핵심 풀이 2~3줄" }`;
    ...
    if (isSelectAll) {
      ...
      //   단, 기존 정답도 ①~⑤ 가 아니면 (예: "0", "5", "5번") 빈 값으로 저장.
