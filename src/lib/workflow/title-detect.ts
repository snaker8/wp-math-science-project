...
export function detectSubjectFromTitle(title: string): string {
  ...
  // ★ 공통수학1/2 로 접지 않고 '수학(상)' 그대로 반환한다 (2026-07-23).
  //   2015 수학(상)은 도형의 방정식(공통수학2), 수학(하)는 경우의 수(공통수학1)를 포함해
  //   어느 쪽도 단일 과목에 안 들어간다. 접어버리면 나머지 절반 유형표를 AI 가 못 봐서
  //   집합·함수 문제가 엉뚱한 대단원에 배정된다(해운대고 실사고).
  //   SUBJECT_CODE_MAP 이 이 라벨을 ['07','08'] 로 펼쳐 두 과목 표를 모두 제공한다.
  if (/수학\s*\(\s*상\s*\)|수학\s*상(?![가-힣])/.test(title)) return '수학(상)';
  if (/수학\s*\(\s*하\s*\)|수학\s*하(?![가-힣])/.test(title)) return '수학(하)';
