...
const SUBJECT_CODE_MAP: Record<string, string | string[]> = {
  ...
  '공수1': '07', '공수2': '08',
  // ★ 2015 개정 수학(상)/(하) — 어느 쪽도 2022 개정 단일 과목에 안 들어간다 (2026-07-23)
  //   수학(상) = 다항식·방정식과부등식(→공통수학1) + **도형의 방정식(→공통수학2)**
  //   수학(하) = 집합과명제·함수(→공통수학2) + **경우의 수(→공통수학1)**
  //   수학비서 트리엔 수학(상)/(하) 과목 자체가 없다(2022 기준 18과목만).
  //   전엔 수학(상)→'07' 단일로 못박아 집합·함수 문제가 볼 유형표가 없어 엉뚱한 대단원
  //   (순열과 조합 등)에 억지 배정됐다(해운대고 수학상 실사고). 두 과목 표를 다 주고
  //   AI 가 고르게 한다 — 학기 불명 중등(['03','04'])과 같은 검증된 배열 경로.
  '수학(상)': ['07', '08'], '수학(하)': ['07', '08'],
  '수학상': ['07', '08'], '수학하': ['07', '08'],
  '대수': '09',
