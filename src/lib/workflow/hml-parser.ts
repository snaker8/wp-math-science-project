...
function renderTableToTabular(
  ...
  // ★ 표 안에 표 — 감싸면 \begin{tabular} 가 중첩되고, 렌더러는 첫 \end{tabular} 에서 짝을
  //   잘못 맞춰 표를 통째로 인라인 텍스트로 흘려버린다.
  //   실사고 (온천중 25-1-2 #21): 대화 전체가 테두리 박스(1열 표)이고 그 안에 x/y 빈칸 표가
  //   들어 있었다. 중첩된 채 저장돼 화면에선 "x 1 3 4 y 75 37.5 30" 한 줄로 뭉개졌고,
  //   **빈칸이 사라져 (1) "표의 빈칸을 채우시오" 가 성립하지 않는 문제**가 됐다.
  //   '' 를 반환하면 호출부가 셀 내용을 본문으로 펼치고, 안쪽 표는 TABLE 분기에서 제대로 복원된다.
  //   바깥 테두리는 잃지만, 표가 깨지는 것보다 낫다.
  if (allCells.some((c) => c.includes('\\begin{tabular}'))) return '';
