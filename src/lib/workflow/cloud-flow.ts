...
import { detectTabularChoices } from '@/lib/workflow/tabular-choices';
 ...
 */
// ★ detectTabularChoices 는 src/lib/workflow/tabular-choices.ts 로 추출 (2026-07-24) —
//   HML 자산화 경로(hml-parser)도 같은 로직을 쓰도록 공용화. 동작 불변.
...
function buildQuestionResult(
  ...
  const dollarDelim = noLbrace
    .replace(/\\left\(/g, '\uE001')
    .replace(/\\right\)/g, '\uE002')
    .replace(/\\\((.+?)\\\)/gs, (_, inner: string) => `$${inner.trim()}$`)
    .replace(/\uE001/g, '\\left(')
    .replace(/\uE002/g, '\\right)');
...
★ 시험지 제목 약어 절대 혼동 금지 (2026-05-26 사고 fix):
  - "공수1" / "공수1" 약어 = "공통수학1" (고1) — 절대 "수학I"·"대수" 로 분류 금지
  - "공수2" / "공수2" 약어 = "공통수학2" (고1) — 절대 "수학II"·"미적분1" 로 분류 금지
  - "수1" 약어 = "수학I" = "대수" (고2)
  - "수2" 약어 = "수학II" = "미적분1" (고2)
  - "공수"(공통수학, 고1) vs "수"(수학I·II, 고2) 의 '공' 한 글자 차이로 학년이 달라짐.
...
- ★ 상용로그 활용 세부유형 구분: "N시간/N년 후 비율/배/배수" + 미생물·인구·세균·재산 같은 실생활 모델 → "실생활" 또는 "주어진 지수식" 노드 우선 (자릿수 X). "매시간/매년 N% 증가·감소" + 실생활 단위 → 거듭제곱 비율 활용. "N자리 수/정수부분이 N자리/몇 자리 수/몇 자리의 자연수" → 자릿수 노드(본문에 명시적으로 "자리"·"자릿수" 단어 있어야). "N의 정수부분·소수부분" → 정수부분 노드. "몇 배인가"·"비율" 표현은 자릿수 X.
- 시그마(Σ)·누적합 = 수열. 극한·연속·미분·적분 = 미적분.

■ 난이도 (수학비서 1~10 스케일, 한국 교육과정 + 정답률 기준)
● 쉬움 (정답률 85%+):
   1 = 개념·정의 그대로 적용. 1단계 풀이.
   2 = 공식 1개 직접 대입. 단순 1~2단계.
● 보통 (정답률 50~85%):
   3 = 공식 1~2개, 2단계 풀이, 기본 응용.
   4 = 표준 응용. 1개념 정공법.
● 어려움 (정답률 22~50%):
   5 = 2개 이상 개념 연결. 3~4단계 풀이.
   6 = 복합 응용·케이스 분석. 표준 합답형(ㄱㄴㄷ).
● 매우어려움 (정답률 22% 미만):
   7 = 다단계 추론. 함정/오개념. 시중 심화서급.
   8 = 여러 개념 융합. 수능 준킬러급(오답률 78%+).
   9 = 수능 킬러급(오답률 95%+). 창의 해법.
   10 = 경시/올림피아드급.

★ 문제 내용으로 정확히 1~10 배정.
★ 합답형/서술형은 자동 어렵게 X — 인지 부담으로 평가:
   · 쉬운 합답형/서술형 (1~2단계 자명) → 3~4
   · 표준 (모든 보기 검증/다단계 풀이) → 5~6
   · 어려운 (개념 융합 + 함정) → 7+
★ 배점이 4점 이상이면 난이도 후보 +1 고려 (확정 X, 참고만).
★ 학년·과정 범위 밖 추론 요구 시 +1.
...
★ difficulty: 1~10 정수 (수학비서 기준)
★ difficultyLabel: "쉬움"(1~2) / "보통"(3~4) / "어려움"(5~6) / "매우어려움"(7~10)
