...
- ★★ 축 눈금 표시: 이미지에 숫자가 표시된 축 좌표를 points에 반드시 포함하세요:
  - x축 위의 숫자(1, 2, π/2 등) → {"x":1, "y":0, "label":"1"} 또는 {"x":1.57, "y":0, "label":"π/2"}
  - y축 위의 숫자(3, -1, -4 등) → {"x":0, "y":3, "label":"3"} 또는 {"x":0, "y":-4, "label":"-4"}
- ★★ 점근선: 이미지에 수평 점선(점근선)이 있으면 expressions에 dashed 스타일로 추가:
  예: 점근선 y=-4 → {"latex":"y=-4", "style":"dashed", "color":"#9ca3af"}
- ★ annotations: 이미지에 써있는 원본 함수 수식(미지수 포함)만 넣으세요:
  예: ["y=\\left(\\frac{1}{4}\\right)^{x-a}+b"] 또는 ["y=a\\sin(bx+c)+d"]
  ★ 점근선 수식(y=-4 등)은 annotations에 넣지 마세요! 점근선은 expressions의 dashed 스타일로만 처리합니다.
...
export async function interpretImage(
  ...
  try {
    ...
    // ★ raw 결과가 photo인지 먼저 확인 (postProcess 전에!)
    const rawFigureType = result.figureType;

    // ★ forceGraph인데 raw 결과가 photo → 다른 Vision 모델로 재시도 (postProcess 전!)
    if (rawFigureType === 'photo' && detected?.forceGraph) {
      const retryProvider = provider === 'claude' ? 'gpt' : 'claude';
      const retryKey = retryProvider === 'claude' ? ANTHROPIC_API_KEY : OPENAI_API_KEY;

      if (retryKey) {
        console.log(`[Vision] ★ ${provider}가 photo 반환 → ${retryProvider}로 재시도 (forceGraph=true)`);
        try {
          const retryResult = retryProvider === 'claude'
          ...
          if (retryResult.figureType !== 'photo') {
            console.log(`[Vision] ★ ${retryProvider} 재시도 성공: ${retryResult.figureType}`);
            return postProcessResult(retryResult, detected);
          }
          console.log(`[Vision] ★ ${retryProvider}도 photo 반환 — postProcess fallback 사용`);
        } catch (retryErr) {
          console.warn(`[Vision] ${retryProvider} 재시도 실패:`, retryErr);
        }
      }
    }

    // postProcess는 재시도 이후에 적용 (photo→graph 강제 변환은 최후 수단)
...
function buildUserMessage(
  ...
  if (detected?.equations && detected.equations.length > 0) {
    msg += `\n\n★★★ 경고: expressions에 위에 제시된 문제 수식만 사용하세요. 이미지에서 다른 수식을 읽어내지 마세요. 미지수(a,b,k 등)는 이미지의 그래프 모양(점근선, x절편, y절편, 꼭짓점 등)에서 값을 결정하여 숫자로 대입하세요.`;
    msg += `\n★★★ 필수 검증: 파라미터를 결정한 후, y=0을 대입하여 x절편을 계산하고 이미지의 x축 교점과 반드시 비교하세요. 불일치하면 파라미터를 재조정하세요!`;
  }
