...
- ★★★ **이미지에 실제로 보이는 라벨만 vertices에 포함하세요!**
  - 이미지에 A, B, C만 적혀있으면 → vertices에 A, B, C만
  - 문제 텍스트에서 정의하는 점(H, D, I 등)이라도 **이미지에 라벨이 안 보이면 vertices에 넣지 마세요**
  - lengths의 값도 이미지에 보이는 것만 (α, β, a, b 등)
...
[핵심 목표]
배경의 노이즈(스캔 자국, 그림자, 연필 자국 등)는 완벽히 제거하되, 이미지 내의 도형/그래프/표 구조는 원본과 '완벽하게 똑같이' 재현합니다.

[★★★ 절대 금지]
- 문제 텍스트(질문 문장)를 SVG에 포함하지 마십시오
- 선택지(①②③④⑤)를 SVG에 포함하지 마십시오
- 오직 도형, 그래프, 표, 다이어그램만 그리십시오
...
async function interpretImageWithGLM(
  ...
  const glmSystemPrompt = getSystemPrompt(detected);
  const userMessage = buildUserMessage(context, detected);
  const combinedPrompt = glmSystemPrompt + '\n\n---\n\n' + userMessage + '\n\nJSON으로만 응답하세요.';
