...
const CHOICE_POLLUTION_PATTERNS = [
  ...
  /^\s*[+\-×÷]\s*[A-Za-z(\\$]/,                 // "+Q(", "-R(", "×f(" 연산자 조각
  /^\s*[+\-]\s*\\[A-Za-z]/,                     // "+\frac{...}", "-\sqrt{...}"
  /^\s*[\]\}\)]/,                               // "}...", "]...", ")..." 수식 닫는 괄호로 시작
  /^\s*\\[a-zA-Z]+\s*[\]\}]/,                   // "\right}", "\bigg]" 같은 LaTeX 닫기
  /^\s*\$\s*[가-힣]/,                            // "$의 값은..." 처럼 $ 뒤 한글
  // 본문 키워드 — 어디든 포함되면 본문 조각으로 판정
  /의\s*값/,                                    // "의 값은", "의 값을"
  /구하시오|구하여라|구하면|구해\s*보시오/,      // 구하 류 서술 지시어
  /\(\s*단\s*[,.]/,                             // "(단,", "(단."
  /\\\(\s*단\s*[,.]/,                           // LaTeX 변환 "\(단,"
  /이때|따라서|그러면/,                         // 본문 접속어
  // 배점·점수 표기
  /\[\s*[\d.]+\s*점\s*\]/,                      // "[4.3점]", "[10 점]"
  /\(\s*총\s*\d+\s*점/,                         // "(총 8점, 부분점수..."
  // 물음표 종결 + 뒤에 배점 등 꼬리 허용
  /\?\s*$/,                                     // "값은?" 물음표 끝
  /\?\s*\[[\d.]+점/,                            // "값은? [4.3점]"
];
