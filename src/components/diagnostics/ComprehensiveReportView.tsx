...
import { buildNarrative, unitShortName } from '@/lib/diagnostics/report-narrative';
import type { ComprehensiveReportPayload, VariantResult } from '@/lib/diagnostics/compute-report';
import { LEVEL_TONE, Panel, StatCard, Gauge } from './report-primitives';
import { REPORT_KIND_LABEL } from '@/lib/report/report-kind';
    ...
    <div className="space-y-5">
      ...
      <div className="flex items-center gap-2 flex-wrap">
        <span className="inline-flex items-center gap-1.5 rounded-full border border-zinc-700 bg-zinc-800/70 px-3 py-1 text-xs font-bold text-zinc-200">
          <span className="text-sm leading-none">{REPORT_KIND_LABEL.diagnostic.emoji}</span>
          {REPORT_KIND_LABEL.diagnostic.name}<span className="text-zinc-500">·</span><span className="text-zinc-400 font-semibold">{REPORT_KIND_LABEL.diagnostic.analysis}</span>
        </span>
      </div>
